 ...
 * Verifies all friend utility behaviors using typed Supabase mocks.
 */
...
// Setup Jest mock for Supabase client
...
// A minimal interface describing the chained methods we need
...
describe("friends_utils", () => {
  let mockChain: SupabaseChainMock;

  beforeEach(() => {
    // Create mock chain with all needed methods
    mockChain = {
      select: jest.fn(),
      insert: jest.fn(),
      update: jest.fn(),
    ...
    // Define default chaining behavior
    ...
    // Tell supabase.from() to return our mock chain
  ...
  describe("getFriends", () => {
    ...
    it("identifies friend when user is addressee", async () => {
      const mockData = [
        {
          id: "f-001",
          status: "accepted",
  ...
  describe("sendFriendRequest", () => {
    it("inserts new pending request successfully", async () => {
      ...
      expect(mockChain.insert).toHaveBeenCalledWith([
        { requester_id: "user-1", addressee_id: "user-5", status: "pending" },
      ]);
  ...
  describe("removeFriend", () => {
    ...
    it("throws if deletion fails", async () => {
      ...
      await expect(removeFriend("f-006", "user-1")).rejects.toThrow("Delete failed");
    });
  });
});
